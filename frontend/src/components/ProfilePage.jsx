...
import { ArrowLeft, LogOut, Plus, Trash2, Eye, Leaf, ShoppingBag, Handshake, Award, CheckCircle } from 'lucide-react';
...
const ProfilePage = () => {
    ...
    const fetch = async () => {
      try {
        ...
        if (profileData?.status !== 'ACTIVE') {
          const appealsResponse = await axios.get(`${API_BASE}/api/appeals/me`, {
  ...
  const handleMarkStatus = async (id, status) => {
    setMarkingId(id);
    try {
      const otpRequest = await axios.post(
        `${API_BASE}/api/products/${id}/status/request-otp`,
        { status },
        { headers: { Authorization: `Bearer ${getToken()}` } }
      );

      const otp = window.prompt(
        `${otpRequest.data?.message || 'Enter the OTP sent to your email'}\n\nEnter 6-digit OTP:`,
        ''
      );

      if (!otp) {
        alert('Status update canceled. OTP not provided.');
        return;
      }

      const res = await axios.post(
        `${API_BASE}/api/products/${id}/status/confirm-otp`,
        { status, otp: otp.trim() },
        { headers: { Authorization: `Bearer ${getToken()}` } }
      );

      // If credits were earned, show toast
      if (res.data.creditsEarned) {
        setCreditToast(`+${res.data.creditsEarned} Green Credits earned! 🌿`);
        setTimeout(() => setCreditToast(''), 3500);
      }
      // Remove or update the listing in local state
      setUserListings(prev => prev.filter(l => l._id !== id));
      // Refresh profile to update greenCredits count
      const updated = await axios.get(`${API_BASE}/api/users/profile`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      ...
      if (retryAfter) {
        alert(`Please wait ${retryAfter}s before requesting another OTP.`);
        return;
      }
  ...
  const handleSubmitAppeal = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE}/api/appeals`,
        { reason: appealReason.trim() },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setAppealMessage(response.data?.message || 'Appeal submitted successfully.');
      setAppealReason('');
  ...
  return (
    <div className="min-h-screen pt-6 pb-20" style={{ background: 'linear-gradient(180deg,var(--bg) 0%,#edf7f0 100%)' }}>
      <div className="container-xl">
        ...
        {/* Green-credits toast */}
        {creditToast && (
          <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[300] rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-xl"
        ...
        {userProfile?.status !== 'ACTIVE' && (
          <div className="glass-card mb-6 p-7">
            <h2 className="text-xl font-extrabold text-[var(--fg)]">Account Appeal</h2>
            <p className="mt-1 text-sm text-[var(--fg-muted)]">
              Your account status is <span className="font-bold">{userProfile?.status}</span>. You can submit an appeal for admin review.
            </p>

            {appealError && <p className="mt-3 text-sm text-rose-600">{appealError}</p>}
            {appealMessage && <p className="mt-3 text-sm text-emerald-600">{appealMessage}</p>}

            <form className="mt-4 space-y-3" onSubmit={handleSubmitAppeal}>
              <textarea
                rows={4}
                value={appealReason}
              ...
              />
              <button type="submit" disabled={appealLoading} className="btn-primary px-4 py-2 text-sm">
                {appealLoading ? 'Submitting…' : 'Submit appeal'}
              </button>
            </form>

            <div className="mt-5">
              <h3 className="text-sm font-bold text-[var(--fg)]">My Appeals</h3>
              {appeals.length === 0 ? (
                <p className="mt-2 text-sm text-[var(--fg-muted)]">No appeals submitted yet.</p>
              ) : (
                <div className="mt-2 space-y-2">
                  {appeals.map((appeal) => (
                    <div key={appeal._id} className="rounded-xl border border-[var(--border)] bg-[var(--bg)] p-3">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-xs font-semibold text-[var(--fg)]">Appeal #{appeal._id}</span>
                        <span className="text-xs font-bold text-[var(--primary)]">{appeal.status}</span>
                      </div>
                      <p className="mt-1 text-sm text-[var(--fg-muted)]">{appeal.reason}</p>
                      <p className="mt-1 text-xs text-[var(--fg-subtle)]">Submitted {new Date(appeal.createdAt).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              )}
                    ...
                    {listing.status === 'AVAILABLE' && (
                      <>
                        <button
                          ...
                          disabled={markingId === listing._id}
                          title="Mark as Sold"
                          className="flex h-8 items-center gap-1 rounded-lg border border-blue-200 bg-white px-2 text-xs font-semibold text-blue-600 transition hover:bg-blue-50 disabled:opacity-50">
                          <CheckCircle size={12} /> Sold
                        </button>
                        {(listing.listingType === 'DONATE' || listing.listingType === 'donate') && (
                          <button
                            onClick={() => handleMarkStatus(listing._id, 'DONATED')}
                            disabled={markingId === listing._id}
                            title="Mark as Donated"
                            className="flex h-8 items-center gap-1 rounded-lg border border-emerald-200 bg-white px-2 text-xs font-semibold text-emerald-600 transition hover:bg-emerald-50 disabled:opacity-50">
                            <Leaf size={12} /> Donated
                          </button>
                        )}
                      </>
                    )}
